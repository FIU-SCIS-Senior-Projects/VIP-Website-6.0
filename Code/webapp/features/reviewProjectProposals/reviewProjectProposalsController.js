(function () {
    ...
    function reviewProjectCtrl($window,$state, $scope, reviewPPS,ToDoService,User, ProjectService) {
		...
		function AcceptProject(projectid, owner, owner_name, title,email,rank,description, image, term, firstSemester, maxStudents,proposedDate)
		{
			...
			// User Story #1207
			var log = {projectid: projectid, student: owner, firstName: owner_name, lastName: owner_name, fullName: owner_name, studentemail: email, selectProject: title, description: description, image: image, term: term, minStudents: firstSemester, maxStudents: maxStudents, action: "Approved", type: "project",proposedDate: proposedDate};
		...
		function RejectProject(projectid,owner, owner_name, title,email,rank, description, image, term, firstSemester, maxStudents,proposedDate)
        {
            reviewPPS.RejectProjects(projectid).then(function(data){
				...
				console.log(owner_name);
				var log = {projectid: projectid, student: owner, firstName: owner_name, lastName: owner_name, fullName: owner_name, studentemail: email, selectProject: title, description: description,image: image,term: term, minStudents: firstSemester, maxStudents: maxStudents, action: "Rejected", type: "project",proposedDate: proposedDate};
				reviewPPS.createLog(log).then(function(success)  {
		...
		function UndoProject(projectid, logid, action, ownerid, owner_name, title, email, desc, image, term, minStud, maxStud,proposedDate)
		{
			...
			var proj = {owner: ownerid, title: title, owner_email: email, owner_rank: "", owner_name: owner_name, firstSemester: minStud, maxStudents: maxStud, description: desc, status: "pending", image: image, term: term,proposedDate: proposedDate};
